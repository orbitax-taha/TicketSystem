...
//               {['Sr.No', 'Title', 'Description', 'Assigned To', 'Status', 'Priority','Created At', 'Key'].map((header, idx) => (
...
//                     <td style={cellStyle}>{ticket.priorityName}</td>
...
const TicketTable = ({ tickets = [], setTickets, refetchTickets }) => {
  ...
  useEffect(() => {
    ...
    if (sortConfig.key) {
      updatedTickets.sort((a, b) => {
        const aValue = sortConfig.key === 'priorityName'
          ? a.priorityName || 'Not set'
          : a[sortConfig.key] || '';
        const bValue = sortConfig.key === 'priorityName'
        ...
        <table style={tableStyle}>
          ...
          <tbody>
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((ticket, idx) => (
                <tr key={idx} style={{ fontWeight: 'bold' }} onClick={() => setSelectedTicket(ticket)}>
                  <td style={cellStyle}>{ticket.srNo}</td>
                  <td style={cellStyle}>{ticket.title}</td>
                  <td style={cellStyle}>{ticket.description?.slice(0, 20)}</td>
                  <td style={cellStyle}>{assigneeDisplayMap[ticket.assignedTo?.toLowerCase()] || ticket.assignedTo}</td>
                  <td style={cellStyle}>{ticket.status}</td>
                  <td style={cellStyle}>{ticket.priorityName}</td>
                  <td style={cellStyle}>{ticket.createdAt}</td>
                  <td style={cellStyle}>{ticket.id}</td>
