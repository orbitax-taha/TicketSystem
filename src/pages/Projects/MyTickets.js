...
//         //  suggestedSLA: ticket.suggestedSLA,
//         //  fileUrl: ticket.ticketAttachment?.[0]?.fileUrl,
...
const MyTickets = () => {
  ...
  const fetchTickets = async () => {
    ...
    try {
      const response = await axiosInstance.get(`/tickets/assignee/username/${userName}`);
      const ticketData = response.data.data || [];
      console.log('Raw ticket data:', ticketData); // Debug raw API response
      const transformedTickets = ticketData.map((ticket, index) => {
        const fileUrls = Array.isArray(ticket.ticketAttachment)
          ? ticket.ticketAttachment.map(attachment => attachment?.fileUrl).filter(url => url)
          : [];
        console.log(`Ticket ${ticket.id} fileUrls:`, fileUrls); // Debug fileUrls per ticket
        return {
          srNo: index + 1,
          ticketCode: ticket.ticketCode,
          id: ticket.id,
      ...
      console.log('Transformed tickets:', transformedTickets); // Debug transformed tickets
      setTickets(transformedTickets);
    } catch (error) {
      console.error('Fetch tickets error:', error.response?.data || error.message);
      Swal.fire({
  ...
  const handleCreateTicket = (newTicket) => {
    const transformedTicket = {
      ...
      suggestedSLA: newTicket.suggestedSLA,
      fileUrls: Array.isArray(newTicket.ticketAttachment)
        ? newTicket.ticketAttachment.map(attachment => attachment?.fileUrl).filter(url => url)
        : [],
    };
    setTickets([transformedTicket, ...tickets]);
    fetchTickets();
  ...
  return (
    <div style={{ display: 'flex' }}>
      <Sidebar />
      <div style={{ flex: 1 }}>
        <Header setShowTicketForm={setShowTicketForm} currentPage="myTickets" />
        {isLoading ? (
          <div style={{ padding: '20px', textAlign: 'center', marginTop: '60px' }}>
            Loading tickets...
          </div>
